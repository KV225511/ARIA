class ProsodyBaselineManager:
    ...
    def _compute_baseline(self,state)->dict[str,float] | None:
        ...
        pitch_baseline=float(sum(baseline_turns[i].get("pitch_mean",0.0) for i in range(len(baseline_turns)))/n)
        rate_baseline=float(sum(baseline_turns[i].get("speech_rate",0.0) for i in range(len(baseline_turns)))/n)
        energy_baseline=float(sum(baseline_turns[i].get("energy_mean",0.0) for i in range(len(baseline_turns)))/n)
